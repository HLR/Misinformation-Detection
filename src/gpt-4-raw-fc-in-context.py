...
result = pd.read_csv("./data/rawfc-in-context-test-results.csv")
...
for idx in tqdm(range(len(merged["claim"]))):
    ...
    if result["in-context"][idx] in ["true","false","half-true"] and False:
...
merged.to_csv("./data/rawfc-in-context-test-results.csv")


# merged = merged[merged["in-context"] != "none"]
